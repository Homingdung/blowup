...
L = 200
mesh = PeriodicRectangleMesh(L, L, 2, 1, direction="x")
...
# initial condition
u_top= as_vector([1.5, 0])
u_bottom = as_vector([-1.5, 0])
u_init = conditional(y0>=0.5, u_top, u_bottom)

    
eps = 0.0795774715495
B_init = as_vector([tanh(y0/eps), 0])
...
bcs1 += DirichletBC(Z1.sub(0), u_bottom, (1,))
bcs1 += DirichletBC(Z1.sub(0), u_top, (2,))

bcs2 = [DirichletBC(Z2.sub(index), 0, subdomain) for index in range(len(Z2)) for subdomain in dirichlet_ids]
bcs2 += DirichletBC(Z2.sub(4), B_init, (1, 2,))
