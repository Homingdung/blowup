# 3D MHD 
# reproduce Dahlburg-MDT-1987
# turbulence relaxation
...
Bz = 0.2 * cos(x) * cos(y)
B_init = as_vector([0, 0, Bz])

uz = 0.4 * cos(x) * cos(y)
u_init = as_vector([0, 0, uz])
jz = sin(x) * sin(y) * (10 - 9 * exp(-3 * (x-0.5 * pi) **2 - 3 * (y - 0.5 * pi) **2))
j_init = as_vector([0, 0, jz])

z.sub(0).interpolate(u_init)
z.sub(6).interpolate(B_init)
z.sub(3).interpolate(j_init)
z_prev.assign(z)
...
j_.rename("Current")
pvd = VTKFile("output/turbulence-relaxation.pvd")
